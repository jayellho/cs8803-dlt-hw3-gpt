...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        # Ensure sequence length does not exceed the model's maximum
        # assert seq_len <= self.seq_length, "Sequence length exceeds model's maximum length."

        # Token embeddings
        token_embeddings = self.transformer.wte(idx)  # (batch_size, seq_len, d_model)

        # Position embeddings
        position_ids = torch.arange(seq_len, dtype=torch.long, device=idx.device)
        position_embeddings = self.transformer.wpe(position_ids)  # (seq_len, d_model)
        position_embeddings = position_embeddings.unsqueeze(0).expand(batch_size, -1, -1)  # (batch_size, seq_len, d_model)
        ...
        x = self.transformer.ln_f(x)  # (batch_size, seq_len, d_model)

        # Output layer to get logits
        logits = self.output_head(x)  # (batch_size, seq_len, n_digits)
    ...
    def inference(self, ids, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # --- TODO: start of your code ---
            curr_seq_len = ids.size(1)
            if curr_seq_len > self.seq_length:
                idx_cond = ids[:, -self.seq_length:]
            else:
                idx_cond = ids
                
            logits, _ = self.forward(idx_cond)
            logits = logits[:, -1, :]  # Focus on the last time step
            next_token = torch.argmax(logits, dim=-1, keepdim=True)  # Determine the next token
            ids = torch.cat((ids, next_token), dim=1)  # Append the next token to the sequence
            # --- TODO: end of your code ---
            pass
